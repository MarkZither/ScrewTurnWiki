...
using ScrewTurn.Wiki.Plugins.Diagram;

namespace DiagramPlugin_Tests
{
	[TestClass]
	public class FileFormatTest
	{
		[TestMethod]
		public void when_mm_file_included_it_is_recognized()
		{
			const string mmFileExample = @"<dia5 contentFormat=""mm"" version=""1.2""/>
...
<edge COLOR=""#309EFF"" />
<richcontent TYPE=""NOTE"">
<pre>Unlawful incarceration is a bad thing!nHistory of events the inmate has gone through</pre>
</richcontent>
...
<edge COLOR=""#309EFF"" />
			...
			FileFormat format = new FileFormat(mmFileExample);
			Assert.AreEqual(FormatState.MindMap, format.State);
			Assert.IsTrue(format.FileContent.StartsWith("<map"));
