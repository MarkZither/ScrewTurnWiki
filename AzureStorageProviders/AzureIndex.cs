...
namespace ScrewTurn.Wiki.Plugins.AzureStorage {
	...
	public class AzureIndex :IIndex {
		...
		public AzureIndex(IIndexConnector connector) {
		...
		public int StoreDocument(IDocument document, string[] keywords, string content, object state) {
			...
			return connector.SaveDataForDocument(document, contentWords, titleWords, words);
		}
		...
		public void RemoveDocument(IDocument document, object state) {
			...
			connector.DeleteDataForDocument(document);
		}
	...
	/// </summary>
	public class AzureWordFetcher : IWordFetcher {
		...
		public bool TryGetWord(string text, out Word word) {
			return implementation(text, out word);
		}
