...
namespace ScrewTurn.Wiki.Tests {
	...
	public abstract class PagesStorageProviderTestScaffolding {
		...
		protected IHostV40 MockHost() {
			...
			DummyIndexDirectoryProvider dummyIndexDirectoryProvider = new DummyIndexDirectoryProvider();
			dummyIndexDirectoryProvider.SetUp(host, "");
			dummyIndexDirectoryProvider.Init(host, "", "root");
			Expect.Call(collectorsBox.GetIndexDirectoryProvider("root")).IgnoreArguments().Return(new DummyIndexDirectoryProvider()).Repeat.Any();
		...
		//    Assert.IsFalse(prov.SetBackupContent(contentInexistent, 0), "SetBackupContent should return false");
		...
		public void RemoveMessage_KeepReplies_PerformSearch() {
			...
			Assert.AreEqual(1, prov.GetMessageCount(page.FullName));
			Message message = prov.GetMessages(page.FullName)[0];
			Assert.AreEqual("RE: Test2", message.Subject, "Wrong message subject");
			Assert.AreEqual("Blah2", message.Body, "Wromg message body");
		}
		...
		//    Assert.AreEqual(0, prov.GetMessageCount(page.FullName), "Wrong number of messages");
		//}
