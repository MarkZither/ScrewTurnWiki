...
namespace ScrewTurn.Wiki {
	...
	public partial class ProviderSelectorThemes : System.Web.UI.UserControl {
		...
		public void Reload() {
			...
			lstProvidersThemes.Items.Clear();

			int count = 0;
			foreach(IProviderV30 prov in allProviders) {
				if(IsProviderIncludedInList(prov)) {
					string typeName = prov.GetType().FullName;
					lstProvidersThemes.Items.Add(new ListItem(prov.Information.Name, typeName));
					if(typeName == defaultProvider) lstProvidersThemes.Items[count].Selected = true;
					count++;
				}
			}
		}
		...
		public string SelectedProvider {
			get { return lstProvidersThemes.SelectedValue; }
			set {
				lstProvidersThemes.SelectedIndex = -1;
				foreach(ListItem itm in lstProvidersThemes.Items) {
					if(itm.Value == value) {
						itm.Selected = true;
						break;
					}
		...
		/// </summary>
		public bool Enabled {
			get { return lstProvidersThemes.Enabled; }
			set { lstProvidersThemes.Enabled = value; }
		}
		...
		public bool HasProviders {
			get {
				return lstProvidersThemes.Items.Count > 0;
			}
		}
		...
		protected void lstProvidersThemes_SelectedIndexChanged(object sender, EventArgs e) {
