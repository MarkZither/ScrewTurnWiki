...
namespace ScrewTurn.Wiki.SearchEngine.Tests
{
	...
	public class OccurrenceDictionaryTests : TestsBase
	{
		...
		{
			Assert.Throws<ArgumentOutOfRangeException>(() => new OccurrenceDictionary(-1));
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.That(() => dic.Add(null, new SortedBasicWordInfoSet()), Throws.ArgumentNullException);
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.That(() => dic.Add(MockDocument("Doc1", "Doc 1", "d", DateTime.Now), null), Throws.ArgumentNullException);
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.That(() => dic[null], Throws.ArgumentNullException);
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.Throws<IndexOutOfRangeException>(() => { SortedBasicWordInfoSet set = dic[MockDocument("Doc", "Doc", "d", DateTime.Now)]; });
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.That(() => { dic[null] = new SortedBasicWordInfoSet(); }, Throws.ArgumentNullException);
		}
		...
		{
			OccurrenceDictionary dic = new OccurrenceDictionary();
			Assert.Throws<IndexOutOfRangeException>(() => { dic[MockDocument("Doc", "Doc", "d", DateTime.Now)] = new SortedBasicWordInfoSet(); });
		}
		...
		public void CopyTo_ShortArray()
		{
			...
			Assert.Throws<IndexOutOfRangeException>(() => dic.CopyTo(array, 0));
		}
